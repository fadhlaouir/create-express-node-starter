...
async function main() {
  try {
    // Check if project name and directory are provided as command-line arguments
    const projectName = process.argv[2];
    const projectDirectory = process.argv[3];

    // If project name or directory is not provided, prompt the user for input
    ...
    const templatesDir = path.resolve(__dirname, "lib", "project-files");
